...
This report records the Phase 2 pipeline decisions, measured outcomes, and the failure modes that directly informed the Phase 3 redesign.
...
We studied flow-termination events in pressure support ventilation (PSV), focusing on the brief transient that appears when the ventilator switches from inspiratory support to exhalation. The core clinical question was simple: do these short events create meaningful lung stress, and can we estimate that stress from non-invasive waveforms alone? To answer that, we analyzed high-frequency airway pressure and flow signals with validated esophageal pressure (Pes) from seven critically ill patients in the public CCVW-ICU cohort.

The Phase 2 pipeline was protocol-locked and applied segmentation, feature extraction, and multi-model regression to 280 breaths. We compared eight candidate models, enforced leave-one-patient-out validation to prevent leakage, and tracked uncertainty with calibrated Gaussian process intervals. The primary XGBoost model reached MAE 5.151 cmH2O in cross-validation and 3.055 cmH2O on an independent two-patient holdout. Ridge and hierarchical Bayesian models looked stronger inside cross-validation (MAE 2.114 cmH2O, R2 0.755) but were less stable on held-out patients, which highlighted the cohort heterogeneity problem. We then converted the measured clinical distributions into conservative engineering boundary conditions and simulation stress targets for Phase 3. Overall, this report establishes a Pes-grounded benchmark for non-invasive PSV transient estimation while documenting both what worked and where generalization still breaks.
