# IEC 60601-1 Preliminary Assessment (Research Draft)

This checklist is used to identify clause-level gaps during iteration; open fields indicate evidence still pending.

Status: Working draft
Date:
Owner:
Linked blocker: process_evidence_missing:iec60601_1_prelim_complete

## Scope
Preliminary clause-based assessment for research-stage design controls.

## Clause Checklist
| Clause | Topic | Applicability | Current Evidence | Gap | Planned Closure |
|---|---|---|---|---|---|

## Abnormal Operation Considerations

## Mechanical/Electrical Safety Notes

## Thermal and Isolation Notes

## Review Sign-off
- Reviewer:
- Date:
